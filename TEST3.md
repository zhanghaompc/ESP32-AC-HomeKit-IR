...
const uint16_t kIrLed = 4;  // 用于控制红外LED的ESP GPIO引脚23
const uint16_t kRecvPin = 23;  // 用于接收红外信号的ESP GPIO引脚
...
void  ble_server_init() {
    ...
    // pServer = BLEDevice::cre ateServer();
    ...
    bleInitialized = true; // 标记蓝牙已初始化
}

// 发送IP地址到蓝牙
void sendIpAddressOverBluetooth() {
  if (deviceConnected && WiFi.status() == WL_CONNECTED) {
    String ipAddress = WiFi.localIP().toString();
    String ipData = "ip=" + ipAddress;
    
    // 转换为蓝牙可发送的格式
    std::vector<uint8_t> data(ipData.length());
    memcpy(data.data(), ipData.c_str(), ipData.length());
    
    pTxCharacteristic->setValue(data.data(), data.size());
    pTxCharacteristic->notify();
    
    Serial.println("已通过蓝牙发送IP地址: " + ipData);
  }
}
...
void setup() {
  ...
  // 移除原有的蓝牙初始化调用
  // ble_server_init();
  Serial.println("系统初始化完成，等待WiFi连接...");
...
void loop() {
  ...
  // 新增：仅在WiFi连接且蓝牙未初始化时启动蓝牙服务
  if (wifiConnected && !bleInitialized) {
    ble_server_init();
    Serial.println("WiFi连接成功,已启动蓝牙服务");
  }

    if (deviceConnected) {
    // 定期发送温湿度数据
    if (millis() - lastBleDataSend >= BLE_DATA_SEND_INTERVAL) {
      // 构建温湿度数据字符串
      String dataString = "temp=" + String(envTemperature, 1) + ";humidity=" + String(enHumidity, 1);
      
      // 将字符串转换为uint8_t数组
      std::vector<uint8_t> data(dataString.length());
      memcpy(data.data(), dataString.c_str(), dataString.length());
      ...
      Serial.println("已通过蓝牙发送温湿度数据: " + dataString);
    }

 if (deviceConnected && !oldDeviceConnected) {
        oldDeviceConnected = deviceConnected;
        Serial.println("设备已连接，发送IP地址...");
        // 延迟一下确保连接稳定
        delay(1000);
        sendIpAddressOverBluetooth();
    }
